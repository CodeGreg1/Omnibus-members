<?php

namespace Modules\Transactions\Providers;

use Modules\Wallet\Events\WalletAdded;
use Illuminate\Support\ServiceProvider;
use Modules\Wallet\Events\WalletDeducted;
use Modules\Deposits\Events\DepositApproved;
use Modules\Wallet\Events\SendMoneyCompleted;
use Modules\Wallet\Events\WalletExchangeCompleted;
use Modules\Transactions\Listeners\HandleMoneySent;
use Modules\Transactions\Listeners\HandleWalletAdded;
use Modules\Carts\Events\CheckoutSubscrptionCompleted;
use Modules\Withdrawals\Events\WithdrawRequestApproved;
use Modules\Transactions\Listeners\HandleWalletDeducted;
use Modules\Transactions\Listeners\HandleApprovedDeposit;
use Modules\Transactions\Listeners\HandleApprovedWithdraw;
use Modules\Subscriptions\Events\SubscriptionPaymentCompleted;
use Modules\Transactions\Listeners\HandleWalletExchangeCompleted;
use Modules\Transactions\Listeners\HandleSubscriptionPaymentCompleted;
use Modules\Transactions\Listeners\HandleCheckoutSubscriptionCompleted;

class TransactionsServiceProvider extends ServiceProvider
{
    /**
     * @var string $moduleName
     */
    protected $moduleName = 'Transactions';

    /**
     * @var string $moduleNameLower
     */
    protected $moduleNameLower = 'transactions';

    /**
     * Boot the application events.
     *
     * @return void
     */
    public function boot()
    {
        $this->registerTranslations();
        $this->registerConfig();
        $this->registerViews();
        $this->loadMigrationsFrom(module_path($this->moduleName, 'Database/Migrations'));
    }

    /**
     * Register the service provider.
     *
     * @return void
     */
    public function register()
    {
        $this->app->register(RouteServiceProvider::class);

        $this->registerEvents();
    }

    /**
     * Register events.
     *
     * @return void
     */
    public function registerEvents()
    {
        $this->app['events']->listen(
            WithdrawRequestApproved::class,
            HandleApprovedWithdraw::class
        );
        $this->app['events']->listen(
            DepositApproved::class,
            HandleApprovedDeposit::class
        );
        $this->app['events']->listen(
            WalletExchangeCompleted::class,
            HandleWalletExchangeCompleted::class
        );
        $this->app['events']->listen(
            SendMoneyCompleted::class,
            HandleMoneySent::class
        );
        $this->app['events']->listen(
            WalletAdded::class,
            HandleWalletAdded::class
        );
        $this->app['events']->listen(
            WalletDeducted::class,
            HandleWalletDeducted::class
        );
        // $this->app['events']->listen(
        //     CheckoutSubscrptionCompleted::class,
        //     HandleCheckoutSubscriptionCompleted::class
        // );
        $this->app['events']->listen(
            SubscriptionPaymentCompleted::class,
            HandleSubscriptionPaymentCompleted::class
        );
    }

    /**
     * Register config.
     *
     * @return void
     */
    protected function registerConfig()
    {
        $this->publishes([
            module_path($this->moduleName, 'Config/config.php') => config_path($this->moduleNameLower . '.php'),
        ], 'config');
        $this->mergeConfigFrom(
            module_path($this->moduleName, 'Config/config.php'),
            $this->moduleNameLower
        );
    }

    /**
     * Register views.
     *
     * @return void
     */
    public function registerViews()
    {
        $viewPath = resource_path('views/modules/' . $this->moduleNameLower);

        $sourcePath = module_path($this->moduleName, 'Resources/views');

        $this->publishes([
            $sourcePath => $viewPath
        ], ['views', $this->moduleNameLower . '-module-views']);

        $this->loadViewsFrom(array_merge($this->getPublishableViewPaths(), [$sourcePath]), $this->moduleNameLower);
    }

    /**
     * Register translations.
     *
     * @return void
     */
    public function registerTranslations()
    {
        $langPath = resource_path('lang/modules/' . $this->moduleNameLower);

        if (is_dir($langPath)) {
            $this->loadTranslationsFrom($langPath, $this->moduleNameLower);
        } else {
            $this->loadTranslationsFrom(module_path($this->moduleName, 'Resources/lang'), $this->moduleNameLower);
        }
    }

    /**
     * Get the services provided by the provider.
     *
     * @return array
     */
    public function provides()
    {
        return [];
    }

    private function getPublishableViewPaths(): array
    {
        $paths = [];
        foreach (\Config::get('view.paths') as $path) {
            if (is_dir($path . '/modules/' . $this->moduleNameLower)) {
                $paths[] = $path . '/modules/' . $this->moduleNameLower;
            }
        }
        return $paths;
    }
}
