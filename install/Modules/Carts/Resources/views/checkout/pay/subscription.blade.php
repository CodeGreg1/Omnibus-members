        ...
        <div class="checkout-column">
            <div class="checkout-overview">
                ...
                <div class="d-flex flex-column mt-3">
                    ...
                    @else
                        <span class="text-gray-500">@lang('Subscribe to') {{ setting('app_name') }}
                            @if ($checkout->hasMoreItems())
                                 @lang('and :count more', [
                                    'count' => $checkout->totalItems() - 1
                                 ])
                            @endif
                        </span>
                        @if ($hasTrial)
                            <div class="d-flex align-items-start flex-column">
                                <strong class="text-white text-4xl checkout-total">
                                    Free for {{ $lineItem->checkoutable->trial_days_count }} {{ $lineItem->checkoutable->trial_days_count > 1 ? __('days') : __('day') }}
                                </strong>
                                <span class="text-gray-500 d-block">
                                    Then {{ $breakdown['total'] }} {{ $lineItem->checkoutable->term->getDescription() }}
                                </span>
                            </div>
                        @else
                            <div class="d-flex align-items-center">
                                ...
                                <span class="text-gray-500 ml-2">
                                    @if ($lineItem->checkoutable->isRecurring())
                                        {{ $lineItem->checkoutable->term->title }}
                                    @else
                                        @lang('Lifetime')
                                    @endif
                                </span>
                ...
                <div class="Checkout-OrderDetails-footer {{ $checkout->mode === 'subscription' || $checkout->mode === 'subscription_onetime' ? 'Items-no-margin-left' : '' }}">
