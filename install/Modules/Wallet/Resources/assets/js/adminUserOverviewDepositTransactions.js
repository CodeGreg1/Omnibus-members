...
app.adminUserOverviewDepositTransactions = {};

app.adminUserOverviewDepositTransactions.config = {
    datatable: '#user-overview-deposit-transactions-datatable',
    options: {
        ...
        columns: [
            ...
            },
            {
                ...
                element: function(row) {
                    return $(`<a href="javascript:void(0)" class="btn btn-primary btn-sm btn-user-overview-deposit-transaction-details-show" data-id="${row.id}"><i class="fas fa-eye"></i></a>`);
                }
            }
        ],
        language: {
        ...
        limit: 15,
        query: {
            user: $('#user-overview-deposit-transactions-datatable').data('user'),
            type: 'deposit'
        }
    }
};

app.adminUserOverviewDepositTransactions.initShowDetails = function() {
    const self = this;
    $(document).delegate('.btn-user-overview-deposit-transaction-details-show', 'click', function() {
        var id = $(this).closest('a').data('id');
        const item = app.adminUserOverviewDepositTransactions.tb.findItem('id', parseInt(id));
        if (item) {
...
app.adminUserOverviewDepositTransactions.init = function() {
    app.adminUserOverviewDepositTransactions.tb =  {};
    if(typeof $.fn.JsDataTable !== 'undefined') {
        app.adminUserOverviewDepositTransactions.tb = $(this.config.datatable).JsDataTable(this.config.options);
    }

    app.adminUserOverviewDepositTransactions.initShowDetails();
};

$(document).ready(app.adminUserOverviewDepositTransactions.init());
