...
app.adminUserOverviewWithdrawalTransactions = {};

app.adminUserOverviewWithdrawalTransactions.config = {
    datatable: '#user-overview-withdrawal-transactions-datatable',
    options: {
        ...
        columns: [
            ...
            },
            {
                ...
                element: function(row) {
                    return $(`<a href="javascript:void(0)" class="btn btn-primary btn-sm btn-user-overview-withdrawal-transaction-details-show" data-id="${row.id}"><i class="fas fa-eye"></i></a>`);
                }
            }
        ],
        language: {
        ...
        limit: 15,
        query: {
            user: $('#user-overview-withdrawal-transactions-datatable').data('user'),
            type: 'withdraw'
        }
    }
};

app.adminUserOverviewWithdrawalTransactions.initShowDetails = function() {
    const self = this;
    $(document).delegate('.btn-user-overview-withdrawal-transaction-details-show', 'click', function() {
        var id = $(this).closest('a').data('id');
        const item = app.adminUserOverviewWithdrawalTransactions.tb.findItem('id', parseInt(id));
        if (item) {
...
app.adminUserOverviewWithdrawalTransactions.init = function() {
    app.adminUserOverviewWithdrawalTransactions.tb =  {};
    if(typeof $.fn.JsDataTable !== 'undefined') {
        app.adminUserOverviewWithdrawalTransactions.tb = $(this.config.datatable).JsDataTable(this.config.options);
    }

    app.adminUserOverviewWithdrawalTransactions.initShowDetails();
};

$(document).ready(app.adminUserOverviewWithdrawalTransactions.init());
