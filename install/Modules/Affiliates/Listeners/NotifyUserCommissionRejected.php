...
class NotifyUserCommissionRejected
{
    public $mailer;
    ...
    {
        if (setting('notify_user_rejected_commissions') === 'enable') {
            try {
                $length = strlen($event->model->user->getName());
                $from_user = Str::mask($event->model->user->getName(), '*', 1, $length - 2);

                $this->mailer->template(AffiliateEmailType::COMMISSION_REJECTED)
                    ...
                    ->with([
                        ...
                        'from_user' => $from_user,
                        'reason' => $event->model->rejected_reason ?? 'No reason'
                    ])
                    ->send();
            } catch (\Exception $e) {
                report($e);
