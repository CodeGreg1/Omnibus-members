    ...
    <div class="row">
        <div class="col-lg-6 col-md-6 col-xm-12">
            <div class="card card-form">
                ...
                <form id="admin-categories-edit-form" method="post" data-action="{{ route('admin.categories.update', $categories->id) }}">
                    @method('patch')
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="category_type_id" class="form-control select2" required>
                                        ...
                                        @foreach($categoryTypes as $categoryType)
                                            <option value="{{ $categoryType->id }}" {{ $categories->category_type_id == $categoryType->id ? 'selected' : '' }}>{{ $categoryType->type }} -> {{ $categoryType->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
