...
class Currency
{
     ...
     *
     * @var Modules\AvailableCurrencies\Services\Contracts\DriverInterface
     */
    protected $driver;
     ...
     * Formatter instance.
     *
     * @var Modules\AvailableCurrencies\Services\Contracts\FormatterInterface
     */
    protected $formatter;
     ...
     */
    public function __construct(array $config, FactoryContract $cache, $currency = 'USD')
    {
        $this->config = $config;
    ...
    public function convert($amount, $from = null, $to = null, $format = true)
    {
        ...
        if (!$this->hasCurrency($to)) {
            $to = $this->config('default');
        }

        // Get exchange rates
        $from_rate = $this->getCurrencyProp($from, 'exchange_rate') ?? 1;
        $to_rate = $this->getCurrencyProp($to, 'exchange_rate') ?? 1;
    ...
    public function format($value, $code = null, $include_symbol = true)
    {
        ...
        $value = floatval($this->numberFormat($value, $decimals));
        $value = number_format($value, $decimals, $decimal, $include_symbol ? $thousand : '');
    ...
    public function getCurrencies()
    {
        if ($this->currencies_cache === null) {
            if (config('app.debug', false) === true) {
                ...
                $this->currencies_cache = $this->cache->rememberForever('currency', function () {
     ...
     * @return array
     */
    public function getCurrencyProp($code, $key, $default = null)
    {
        if (!$this->hasCurrency($code)) {
            return $default;
        }
