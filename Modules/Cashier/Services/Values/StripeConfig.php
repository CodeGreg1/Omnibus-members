...
class StripeConfig implements Config
{
    public function get()
    {
        return [
            'title' => 'Card',
            'key' => setting(setting('cashier_mode', 'sandbox') . '_stripe_publishable_key'),
            'secret' => setting(setting('cashier_mode', 'sandbox') . '_stripe_secret_key'),
            'webhook' => [
                'secret' => setting(setting('cashier_mode', 'sandbox') . '_stripe_webhook_secret'),
                'tolerance' => 300
            ],
            'product_key' => setting(setting('cashier_mode', 'sandbox') . '_stripe_product_key'),
            'service_base_namespace' => 'Modules\\Cashier\\Services\\Clients\\Stripe\\Service',
            'payment_methods' => [
                'card'
            ],
            'status' => setting('stripe_status'),
            'logo' => setting('stripe_logo'),
            'name' => setting('stripe_display_name'),
            'view_instance' => [
                'cart' => [
            ...
            'button' => [
                'class' => 'btn btn-success btn-lg mb-4 btn-pay-pricing',
                'href' => '',
                'content' => '<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                        viewBox="0 0 468 222.5" style="enable-background:new 0 0 468 222.5;" xml:space="preserve" height="30">
                    <g>
