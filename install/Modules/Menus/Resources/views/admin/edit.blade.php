    ...
    <div class="row">
        <div class="col-md-8">
            ...
            <div class="card card-form" id="details">
                ...
                <form id="menu-edit-form" method="post" data-action="{{ route('admin.menus.update', $menus->first()->id) }}">
                    @method('patch')
                    @csrf
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" class="form-control select2">
                                        ...
                                        @foreach($menuTypes as $menuType)
                                            <option {{ $menuType==$menus->first()->type?'selected':'' }}>{{ $menuType }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="tab-content tab-bordered" id="language-content">
                                    @foreach($languages as $language)
                                        <div class="tab-pane fade {{ $language->code == 'en' ?'show active':'' }}" id="{{$language->code}}" role="tabpanel" aria-labelledby="{{$language->code}}-tab">
                                            ...
                                            <input type="hidden" name="menu-edit-output[{{$language->code}}]" id="menu-edit-output-{{$language->code}}" class="menu-edit-output form-control">
                                            <div class="custom-dd-empty dd dd-full-width edit-menu-item" id="edit-menu-item-{{$language->code}}" data-lang="{{$language->code}}">
                                                <ol class="dd-list">
                                                    @foreach($menus as $menu)
                                                        @if($menu->language_id == $language->id)
                                                            @if($menus->first()->type == \Modules\Menus\Support\MenuType::FRONTEND)
                                                                {!! HtmlNestableBuilder::parse(HtmlMenuLinksBuilder::parse($menu->frontend_links->toArray())) !!}
                                                            @else
                                                                {!! HtmlNestableBuilder::parse(HtmlMenuLinksBuilder::parse($menu->links->toArray())) !!}
                                                            @endif
                                                        @endif
                                                    @endforeach
                                                </ol>
    ...
    <div class="modal fade" role="dialog" id="add-menu-item-modal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="menu-item-edit-form" method='post' autocomplete='off'>
                    ...
                    <div class="modal-footer bg-whitesmoke br">
                        ...
                        <button type="submit" class="btn btn-primary" id="menu-item-edit-button">@lang('Save changes')</button>
                    </div>
                </form>
            </div>
        </div>
